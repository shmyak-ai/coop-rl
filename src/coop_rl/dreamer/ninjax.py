    ...
    def __setitem__(self, key, value):
        ...
        if not self.modify:
            ...
            raise RuntimeError(
                ...
                f"and dtype {existing.dtype} to shape {value.shape} and " + f"dtype {value.dtype}."
            )
...
def pure(fun, nested=False):
    ...
    @hidestack
    def purified(
        state, *args, seed=None, create=None, modify=None, ignore=None, track=False, **kwargs
    ):
...
def grad(fun, targets, has_aux=False):
    ...
    def wrapper(*args, **kwargs):
        ...
        (y, (changes, aux)), dx = backward(x1, x2, *args, seed=seed(None, True), **kwargs)
...
def while_loop(cond_fun, body_fun, carry):
    ...
    unchanging = {k: v for k, v in context().items() if k in accessed and k not in modified}
    ...
    carry, changes, _ = jax.lax.while_loop(cond_fun_wrapper, body_fun_wrapper, (carry, changing, 0))
...
def scope(name, absolute=False, multi=False):
    ...
    if absolute:
        ...
        _set_scope(outside + "/" + name)
    try:
        yield _get_scope()
    except Exception as e:
        if not hasattr(e, "_njscope"):
            e._njscope = _get_scope()
            add_note(e, f"Exception happened inside Ninjax scope '{_get_scope()}'.")
        raise
    finally:
        _set_scope(outside)
...
def validate(path, single=False):
    ...
    for name in names:
        ...
        assert re.match(r"^[A-Za-z0-9_]+$", name), (
            "Only letters, numbers, and underscores allowed in names",
            path,
            name,
        )
...
class ModuleMeta(type):
    ...
    def __new__(mcs, name, bases, clsdict):
        ...
        for name, typ in cls.__annotations__.items():
            try:
                ...
                raise ValueError(f"Annotation '{typ}' for field '{key}' is not a valid type.")
        cls._defaults = {
            k: getattr(cls, k) for k, v in cls.__annotations__.items() if hasattr(cls, k)
        }
    ...
    def __call__(cls, *args, name=None, **kwargs):
        ...
        if not isinstance(name, str):
            raise TypeError("Please provide a module name via Module(..., name='example').")
        ...
        for key, typ in cls.__annotations__.items():
            if key in kwargs:
                ...
                raise TypeError(f"Pass a keyword arg for field '{key}' or define a default.")
            if typ is not None and not isinstance(value, typ):
                raise TypeError(
                    f"Value '{value}' for field '{key}' is not of type '{typ.__name__}'."
                )
            fields[key] = value
        obj = cls.__new__(cls)
        obj._fields = fields
...
class Module(object, metaclass=ModuleMeta):
    ...
    def value(self, name, make, *args, **kwargs):
        ...
        validate(name)
        assert _get_scope() == self.path, (
            name,
            "Values can only be created in the root scope of a module.",
        )
        path = self.path + "/" + name
    ...
    def sub(self, name, make=None, *args, **kwargs):
        ...
        validate(name)
        current_scope = _get_scope()
        assert current_scope == self.path or current_scope.startswith(self.path + "/"), (
            name,
            "Can only create submodules from inside the parent module.",
        )
        if current_scope == self.path:
            handle = name
        else:
            assert current_scope.startswith(self.path + "/")
            handle = current_scope[len(self.path) + 1 :] + "/" + name
...
def flatten(tree):
    ...
    tostr = lambda x: (
        re.sub(r"[^a-z0-9-_/]+", "_", str(x).lower()).replace("_/", "").replace("_", "")
    )
