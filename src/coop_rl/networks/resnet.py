...
InnerOp = nn.Module | Callable[..., chex.Array]
...
class VisualResNetTorso(nn.Module):
    ...
    def __call__(self, observation: chex.Array) -> chex.Array:
        ...
        channels_blocks_strategies = zip(
            self.channels_per_group, self.blocks_per_group, self.downsampling_strategies, strict=True
        )
...
class ResNetTorso(nn.Module):
    ...
    def __call__(self, observation: chex.Array) -> chex.Array:
        output = observation
        hidden_units_blocks = zip(self.hidden_units_per_group, self.blocks_per_group, strict=True)
