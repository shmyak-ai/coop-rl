...
class QuantileQNetworkHead(nn.Module):
    """Plain (non-dueling) IQN head: quantile fusion -> MLP -> per-action quantile values."""
...
class NoisyDuelingQuantileQNetworkHead(nn.Module):
    """DuelingQuantileQNetworkHead with NoisyLinear dueling streams (Fortunato et al. 2018).

    The IQN cosine-embedding fusion stays a plain Dense (matches BTR's own
    networks.py, where only the dueling streams are wrapped in the noisy/plain
    linear-layer switch, not the cosine embedding).
    """
    ...
    def __call__(
        ...
        value = NoisyLinear(1, sigma_zero=self.sigma_zero, dtype=self.dtype)(value_torso)
        advantages = NoisyLinear(self.action_dim, sigma_zero=self.sigma_zero, dtype=self.dtype)(
            advantages_torso
        )
        z_values = value + advantages - advantages.mean(axis=-1, keepdims=True)
