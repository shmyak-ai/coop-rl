...
from coop_rl.base.environment import HandlerUnityEnv
...
def _args_network():
    """DreamerV3 'size1m' model + actor-critic hyperparameters (Unity path).
...
def get_config():
    ...
    # Run / batching. num_envs MUST equal the number of agents in the Unity scene.
    num_envs = 16
    batch_size = 16
    batch_length = 64
    ...
    config.env = env = HandlerUnityEnv
    config.args_env = args_env = ml_collections.ConfigDict()
    config.args_env.file_name = "/path/to/unity_binary"  # built Unity player executable
    config.args_env.num_envs = num_envs
    config.args_env.base_port = 5005
    config.args_env.no_graphics = False  # visual observations require rendering
    config.args_env.time_scale = 20.0
