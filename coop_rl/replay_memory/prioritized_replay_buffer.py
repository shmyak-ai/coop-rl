...
from coop_rl.replay_memory import sum_tree
from coop_rl.replay_memory.circular_replay_buffer import ReplayElement
import numpy as np
...
class OutOfGraphPrioritizedReplayBuffer(
  ...
  def set_priority(self, indices, priorities):
    ...
    ), f'Indices must be integers, given: {indices.dtype}'
  ...
  def get_priority(self, indices):
    ...
    assert (
        indices.dtype == np.int32
    ), f'Indices must be int32s, given: {indices.dtype}'
